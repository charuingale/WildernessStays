...
Both are complete and identical in features: hotel browsing with filters, room-level booking with photos/amenities/capacity, per-room date blocking (no double-booking, enforced by serializable transactions with row locks), a cancellation policy (free until 7 days before check-in, 70% fee within the final 7 days, no cancellation on or after check-in, with refund tracking), availability calendar, JWT auth with private booking history, admin reservations desk, CSV export, real-time updates, offline demo mode, and a smoke-test suite.
