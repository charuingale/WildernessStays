 ...
 * ASP.NET Core backend. Speaks negotiate → WebSocket → handshake → invocations.
 */
...
/**
 * Connect once; listeners receive real-time booking/availability events.
 * Detects the running backend via /api/health: SignalR for ASP.NET Core,
 * Socket.IO for the Node/NestJS backend.
 */
export function connectSocket(onEvent) {
  if (socket) return socket;
  socket = true; // guard against double-connect while detection runs
  (async () => {
    let engine = 'node';
    try {
      const res = await fetch('/api/health');
      if (res.ok) engine = (await res.json()).engine || 'node';
    } catch { /* default to node */ }
    if (engine === 'aspnet') {
      socket = await connectSignalR(onEvent).catch(() => null);
    } else {
      socket = io('/', { path: '/socket.io', reconnectionAttempts: 5, timeout: 4000 });
